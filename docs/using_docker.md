...
With Docker you can easily set up *btcd* to run your Bitcoin full node. You can find the official *btcd* Docker images on Docker Hub [alexdcox/dashd-go](https://hub.docker.com/r/alexdcox/dashd-go). The Docker source file of this image is located at [Dockerfile](https://github.com/alexdcox/dashd-go/blob/master/Dockerfile).
...
services:
  btcd:
    container_name: btcd
    hostname: btcd
    image: alexdcox/dashd-go:latest
    restart: unless-stopped
    volumes:
      - btcd-data:/root/.btcd
    ports:
